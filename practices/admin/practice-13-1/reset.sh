...
function undeployApplication {
    ...
    echo "Attempting to stop and undeploy ShoppingCart application"
  
    local whichApplication="ShoppingCart"
	...
	#echo curl -v --user ${WLSUsername}:${WLSPassword} -H X-Requested-By:MyClient -X DELETE http://${WLSAdminHost}:${WLSDeployPort}/management/weblogic/latest/edit/appDeployments/$whichApplication #> /dev/null 2>&1
